...
const std::shared_ptr<Player>& World::getMPlayer() const { return m_player; }

const std::set<std::shared_ptr<Platform>>& World::getMPlatforms() const { return m_platforms; }

void World::checkPlayerPlatformCollision()
{
        if (m_player->getMVstate() == Falling) {
                for (const auto& platform : allPlatforms()) {
                        if (m_player->getC().getY() - m_player->getMHeigth() <
                                platform->getC().getY() && // lowest y coord of player is under
                                                           // highest y coord of platform
                            m_player->getC().getY() - m_player->getMHeigth() >
                                platform->getC().getY() - platform->getMHeigth() && // lowest y coord of player is over
                                                                                    // lowest y coord of plat
                            ((m_player->getC().getX() > platform->getC().getX() &&
                             ...
                             (m_player->getC().getX() + m_player->getMWidth() > platform->getC().getX() &&
                              m_player->getC().getX() + m_player->getMWidth() <
                                  platform->getC().getX() + platform->getMWidth()))) {
...
void World::playermove(Horizontalstate state) { m_player->setMHstate(state); }
...
const std::set<std::shared_ptr<VerticalPlatform>>& World::getMVerticalplatforms() const { return m_verticalplatforms; }
...
void World::generateRandomStart()
{
        ...
        for (int i = 0; i < randomplatforms; ++i) {
                Coordinate randomcoord(Random::getInstance()->makerandom(-3.0, 3.0),
                                       Random::getInstance()->makerandom(-3.0, 4.0));
...
bool World::checkHVValidPlatform(const shared_ptr<Platform>& nPlatform)
{
        for (const auto& platform : m_verticalplatforms) { /**Check collision for vertical **/
                if (checkCollision(platform, nPlatform) ||
                    checkXCollision(platform, nPlatform)) { /** Check if there is another platform on
                                                               the same x coord */
                        return false;
                }
        }
        for (const auto& platform : m_horizontalplatforms) { /**Check collision for vertical **/
                if (checkCollision(platform, nPlatform) ||
                    checkYCollision(platform, nPlatform)) { /** Check if there is another platform on
                                                               the same y coord */
                        return false;
                }
        }
...
bool World::checkCollision(const shared_ptr<Object>& obj1, const shared_ptr<Object>& obj2)
...
bool World::checkXCollision(const shared_ptr<VerticalPlatform>& obj1, const shared_ptr<Object>& obj2)
{ /**Check collision for a vertical platform
     with another object **/
        if (obj1->getC().getX() < obj2->getC().getX() + obj2->getMWidth() &&
            obj1->getC().getX() + obj1->getMWidth() > obj2->getC().getX()) {
...
bool World::checkYCollision(const shared_ptr<Object>& obj1, const shared_ptr<Object>& obj2)
{ /**Check collision for a horizontal
     platform with another object **/
...
const shared_ptr<Score>& World::getMScore() const { return m_Score; }

void World::setMScore(const shared_ptr<Score>& mScore) { m_Score = mScore; }
...
const set<std::shared_ptr<BG_Tile>>& World::getMBgtile() const { return m_bgtile; }

void World::setMBgtile(const set<std::shared_ptr<BG_Tile>>& mBgtile) { m_bgtile = mBgtile; }
